...
//            return null;
    ...
    public void calcAvgAndStudentCount(ClassRoom classRoom,Integer id) {
        String query = String.format("UPDATE %s SET number_of_students = (Select count(*) FROM %s WHERE class_id = %d)," +
                "class_avg = (Select avg(avg_grade) FROM %s WHERE class_id = %d) WHERE id = %d",CLASSROOM_TABLE_NAME,StudentRepository.STUDENT_TABLE_NAME,id,StudentRepository.STUDENT_TABLE_NAME,id,id);
